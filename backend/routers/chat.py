                ...
                if len(pattern_words) == 1 and len(pattern) >= 3:
                    for word in msg_words:
                        if _fuzzy_match(word, pattern):
                            fuzzy_score = (len(pattern) / max(len(msg_lower), 1)) * 0.8
...
async def execute_intent(intent: IntentCategory, message: str) -> Dict[str, Any]:
    ...
    if intent == IntentCategory.SYSTEM_STATUS:
        ...
        # ✅ FIXED: Use absolute import for scanner
        from routers.scanner import _check_port, TOP_PORTS
        open_ports = []
        ...
        # ✅ FIXED: Use absolute import for exploit
        ...
        try:
            # ✅ FIXED: Use absolute import for darkweb
        ...
        try:
            # ✅ FIXED: Use absolute import for darkweb
            from routers.darkweb import _robin_available, _llm_available, _check_tor, SEARCH_ENGINES
            tor = _check_tor()
            return {
        ...
        # ✅ FIXED: Use absolute import for workflow_engine
        ...
        # ✅ FIXED: Use absolute import for remote_ops
        ...
        # ✅ FIXED: Use absolute import for linux_shell
        ...
        if any(k in msg_lower for k in ["linux status", "shell status", "terminal status", "wsl status"]):
            ...
            # ✅ FIXED: Use execute_script from linux_shell
