...
@router.get("/summary")
@geo_router.get("/summary")
async def get_geo_summary():
    """Returns overall summary of Generative Engine Optimization (GEO) tracked citations."""
    try:
        queries = await CitationService.get_tracked_queries()
        total_tracked = len(queries)
        total_citations = sum(q.get("citation_count", 0) for q in queries)
        avg_sov = round(sum(q.get("share_of_voice", 0.0) for q in queries) / max(1, total_tracked), 2)
        return {
            "total_tracked_queries": total_tracked,
            "total_citations_verified": total_citations,
            "avg_share_of_voice": avg_sov,
            "geo_engines_monitored": ["ChatGPT", "Perplexity AI", "Claude", "Gemini", "Bing Copilot"],
