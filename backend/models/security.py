...
class SecurityEngagement(Base):
    ...
    # One-time bulk approval mode
    auto_approved = Column(Boolean, default=False)
    auto_approved_by = Column(String, nullable=True)
...
class EngagementPhaseLog(Base):
    ...
    # ─── STYX PRIME DETECTION MODELS ────────────────────────────────

class STYXDetection(Base):
    """Stores detection events for STYX PRIME threats."""
    __tablename__ = "styx_detections"

    id = Column(String, primary_key=True, default=_gen_uuid)
    network_scope = Column(String(64), nullable=False)
    detection_time = Column(DateTime, default=datetime.utcnow)
    device_ip = Column(String(64), nullable=False)
    detection_pattern = Column(String(64), nullable=False)   # NTP_ANOMALY, BMC_SPOOF, etc.
...
class STYXNode(Base):
    """Tracks infected nodes and their propagation paths."""
    __tablename__ = "styx_nodes"

    id = Column(String, primary_key=True, default=_gen_uuid)
    node_id = Column(String(64), nullable=False, unique=True)
    ip_address = Column(String(64), nullable=False)
    last_seen = Column(DateTime, default=datetime.utcnow)
    infection_status = Column(String(32), nullable=False)    # INFECTED, SUSPECT, CLEAN
    node_type = Column(String(32), nullable=False)           # firewall, BMC, host, infected_parent
    parent_node = Column(String(64), nullable=True)


class STYXReport(Base):
    """Stores generated STYX threat reports."""
    __tablename__ = "styx_reports"
