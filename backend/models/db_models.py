...
import uuid
from sqlalchemy import Column, String, Float, DateTime, Boolean, ForeignKey, JSON, Text, Integer
...
class SelfHealingLogModel(Base):
    __tablename__ = "self_healing_logs"
    id = Column(String, primary_key=True, default=lambda: gen_id("SH"))
    error_type = Column(String, nullable=False)
    endpoint = Column(String, nullable=True)
    traceback_summary = Column(Text, nullable=False)
    proposed_patch = Column(Text, nullable=False)
    status = Column(String, default="diagnosed")  # diagnosed, applied, ignored
    created_at = Column(DateTime, default=datetime.utcnow)


class SystemRuleModel(Base):
