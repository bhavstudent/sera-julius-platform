...
if ENTITY_MODE == "mock":
    ...
    # Live mode: inject demo key for development unless explicitly disabled.
    # In production deployments, set DISABLE_DEMO_KEY=true to remove it.
    if os.getenv("DISABLE_DEMO_KEY", "").lower() in ("true", "1", "yes"):
        DEMO_API_KEY = None
    else:
        DEMO_API_KEY = _demo_key_env or _demo_key_default
...
def _submit_async_job(coro_fn):
    ...
    if _main_event_loop and _main_event_loop.is_running():
        asyncio.run_coroutine_threadsafe(coro_fn(), _main_event_loop)
    else:
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
...
async def startup():
    ...
    # and drives autonomous entity discovery so the dashboard is always alive.
    try:
        from services.live_engine import start_live_engine
        start_live_engine()
        logger.info("[STARTUP] Real-time live activity engine started.")
    except Exception as e:
        logger.error(f"[STARTUP] Failed to start live engine: {e}", exc_info=True)
    ...
    # ─── AUTONOMOUS STYX SCANNER ─────────────────────────────────────
    # Start the scanner only if enabled via environment variable
    if os.getenv("AUTONOMOUS_SCANNER_ENABLED", "false").lower() == "true":
        try:
            logger.info("[STARTUP] Starting autonomous STYX scanner...")
            scanner = AutonomousScanner()
            app.state.autonomous_scanner = scanner
            asyncio.create_task(scanner.start())
            logger.info("[STARTUP] Autonomous STYX scanner started.")
        except Exception as e:
    ...
    if os.getenv("EXTERNAL_SCANNER_ENABLED", "false").lower() == "true":
        try:
            logger.info("[STARTUP] Starting external network orchestrator...")
            from services.external_orchestrator import ExternalNetworkOrchestrator
            external_orch = ExternalNetworkOrchestrator()
            app.state.external_orchestrator = external_orch

            async def run_external_cycle():
                while True:
                    try:
                        await external_orch.run_cycle()
                    except Exception as e:
                        logger.error(f"[EXTERNAL] Cycle error: {e}")
                    await asyncio.sleep(int(os.getenv("EXTERNAL_SCAN_INTERVAL", "3600")))

            asyncio.create_task(run_external_cycle())
            logger.info("[STARTUP] External network orchestrator started.")
        except Exception as e:
            logger.error(f"[STARTUP] Failed to start external orchestrator: {e}")
    else:
...
async def shutdown():
    await close_redis()
    if hasattr(app.state, "autonomous_scanner") and app.state.autonomous_scanner:
        app.state.autonomous_scanner.stop()
        logger.info("[SHUTDOWN] Autonomous STYX scanner stopped.")
