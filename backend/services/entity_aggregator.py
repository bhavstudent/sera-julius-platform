...
class EntityAggregator:
    ...
    async def get_full_profile(cls, ticker: str) -> dict:
        ...
        async with async_session_maker() as session:
            ...
            if not company:
                # ── On-The-Fly Dynamic Entity Auto-Resolver ──
                # Automatically register any requested company ticker on-the-fly with real-time profile data
                clean_ticker = ticker.upper().strip()
                company_name = f"{clean_ticker} Corporation"
                
                # Known legal names mapping for top tickers
                NAMES_MAP = {
                    ...
                    "INTC": "Intel Corporation", "PLTR": "Palantir Technologies Inc.", "CRWD": "CrowdStrike Holdings, Inc.",
                    "PANW": "Palo Alto Networks, Inc.", "ORCL": "Oracle Corporation", "IBM": "International Business Machines Corp."
                }
                if clean_ticker in NAMES_MAP:
                    company_name = NAMES_MAP[clean_ticker]
                
                company = CompanyModel(
                    id=f"CO-{clean_ticker}",
                    ticker=clean_ticker,
                ...
                try:
                    ...
                    logger.warning(f"[AGGREGATOR] Auto-registration rollback for {clean_ticker}: {e}")
