...
class JuliusIntegrationBridge:
    ...
    def _print_status(self):
        ...
        print(f"  AXIOM (Compression):     {'? OPERATIONAL' if axiom_ok else '?? LIMITED'}")
        print(f"  KRONOS (Scaling):        {'? OPERATIONAL' if kronos_ok else '?? LIMITED'}")
        print(f"  Causal Functor:          {'? OPERATIONAL' if causal_ok else '?? LIMITED'}")
