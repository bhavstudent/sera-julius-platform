          ...
          <GlassCard title="Track New SEO Query" subtitle="Define a target query to monitor in generative engine listings">
            <form onSubmit={handleCreateQuery} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                >
                  {entities.map(e => (
                    <option key={e.id} value={e.name} style={{ background: '#08080c', color: 'var(--text-primary)' }}>
                      {e.name}
                    </option>
                  ))}
                </select>
              ...
              {rateLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '10px 0' }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: '28px', fontWeight: 'bold', color: 'var(--crimson)', fontFamily: 'monospace' }}>
                  ...
                  <div className="mono" style={{ fontSize: '12px', width: '100%', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.03)', paddingBottom: '4px' }}>
                      ...
                      <span style={{ color: 'var(--crimson)', fontWeight: 'bold' }}>{rateData.cited_checks}</span>
                    </div>
          ...
          <GlassCard title="Tracked Query Catalog" subtitle={`Active tracked query strings: ${queries.length}`}>
            {loadingQueries ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {queries.map((q, idx) => (
                  <div
                    key={q.query_id || q.id || `q-${idx}`}
                    style={{
                      padding: '16px',
                      borderRadius: '6px',
                      background: 'rgba(12, 12, 18, 0.75)',
                      border: selectedQueryForHistory?.query_id === q.query_id ? '1px solid var(--crimson)' : '1px solid var(--border)',
                      display: 'flex',
                      flexDirection: 'column',
                      gap: '12px'
                    }}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                      <div>
                        <span className="mono" style={{ fontSize: '10px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>
                          ID: {q.query_id} | Target: <strong style={{ color: 'var(--crimson)' }}>{q.target_entity_name}</strong>
                        </span>
